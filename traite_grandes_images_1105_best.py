...
reseau = DeeplabV3Plus(num_classes=4)
reseau.load_state_dict(torch.load('C:/Bloodcells/Modele/Data/ALL_IDB2/ALL_IDB2/img_modified/img_modified/Pytorch1205_4_classes.pth'))
...
def process_image(img_path, tile_size, overlap, model):
    ...
    new_size = tuple(int(1.4*x) for x in img_size)
...
img_paths = glob.glob("C:/Bloodcells/Modele/Data/ALL_IDB1/ALL_IDB1/im/*.jpg")
...
labels = ['Background', 'Normal', 'Smudge', 'Blaste']
...
#     plt.imshow(colored_image, alpha=1.0) # Définir une transparence pour contrôler la luminosité de l'image
...
for img_path in img_paths:
    ...
    # Enregistrer l'image de Label après une rotation de 270 degrés
    output.save(label_path)
    # Concaténer l'image d'origine à gauche de l'image résultante
    colored_image = Image.new('RGB', (original_width + output.width, original_height))
    colored_image.paste(original_image, (0, 0))
    colored_image.paste(output.resize((original_width,original_height)), (original_width, 0))
    # Création d'une liste de patches avec les couleurs correspondantes
    patches = [Patch(color=c/255, label=labels[i]) for i, c in enumerate(colors)]
    # Configuration de l'affichage du graphique
    plt.figure(figsize=(10, 10))
